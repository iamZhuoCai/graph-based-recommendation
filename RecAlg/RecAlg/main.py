...
if __name__ == '__main__':
    ...
    if args.model_type == 'NGCF':
        norm_dia_adj = data_generator.get_dia_adj_mat()
        model = NGCF(data_generator.n_users,
                     data_generator.n_items,
                     norm_dia_adj, args).to(args.device)
        ...
        model = LightGCN(data_generator.n_users,
                     data_generator.n_items,
                     norm_dia_adj, args).to(args.device)
    elif args.model_type == 'LRGCCF':
        model = LRGCCF(data_generator.n_users,
                     data_generator.n_items,
                     norm_dia_adj, args).to(args.device)
    elif args.model_type == 'GCMC':
        norm_adj = data_generator.get_adj_mat()
        model = GCMC(data_generator.n_users,
                     data_generator.n_items,
                     norm_adj, args).to(args.device)
    elif args.model_type == 'SCF':
        norm_left_adj = data_generator.get_left_adj_mat()
        model = SCF(data_generator.n_users,
                     data_generator.n_items,
                     norm_left_adj, args).to(args.device)
    elif args.model_type == 'BPR':
        model = BPR(data_generator.n_users,
                     data_generator.n_items, args).to(args.device)
